...
import file.FileExtensions;
import file.RecentFilesQueue;
import utils.UserPreferences;
import utils.UserPreferences.booleanPreference;
import diagram.Diagram;
import ui.tips.TipDialog;
import file.DeserializationException;
import file.PersistenceService;
import file.VersionedDiagram;
import viewers.ImageCreator;
...
public class EditorFrame extends BorderPane
{
	...
	public EditorFrame(Stage pMainStage, Optional<File> pOpenWith) 
	{
		...
		aRecentFiles.deserialize(Preferences.userNodeForPackage(MetaBuilder.class).get("recent", "").trim());
	...
	private void createFileMenu(MenuBar pMenuBar, List<NewDiagramHandler> pNewDiagramHandlers) 
	{
		...
		{
			newMenu.getItems().add(factory.createMenuItem("metamodeldiagram", false, handler));
		}
	...
	private void createHelpMenu(MenuBar pMenuBar) 
	{
		...
		pMenuBar.getMenus().add(factory.createMenu("help", false,
				...
				factory.createMenuItem("help.guide", false, event -> MetaBuilder.openBrowser(USER_MANUAL_URL)),
	...
	private List<NewDiagramHandler> createNewDiagramHandlers()
	{
		...
		result.add(new NewDiagramHandler(pEvent ->
		{
			insertGraphFrameIntoTabbedPane(new DiagramTab(new Diagram()));
		}));
		return Collections.unmodifiableList(result);
	}
	...
	private void openFile() 
	{
		...
		fileChooser.getExtensionFilters().add(FileExtensions.getFilter());
	...
	private void saveAs() 
	{
		...
		FileChooser fileChooser = new FileChooser();
		fileChooser.getExtensionFilters().add(FileExtensions.getFilter());
		fileChooser.setSelectedExtensionFilter(FileExtensions.getFilter());
	...
	{
		String dir = Preferences.userNodeForPackage(MetaBuilder.class).get(pKey, ".");
	...
	{
		Preferences.userNodeForPackage(MetaBuilder.class).put(pKey, pLastExportDir.getAbsolutePath().toString());
	}
	...
	private void exportImage() 
	{
		FileChooser fileChooser = getImageFileChooser(getLastDir(KEY_LAST_EXPORT_DIR), 
				Preferences.userNodeForPackage(MetaBuilder.class).get(KEY_LAST_IMAGE_FORMAT, "png"));
		...
		Preferences.userNodeForPackage(MetaBuilder.class).put(KEY_LAST_IMAGE_FORMAT, format);
	...
	public void exit() 
	{
		...
		if (modcount > 0) 
		{
			...
			if (alert.getResult() == ButtonType.YES) 
			{
				Preferences.userNodeForPackage(MetaBuilder.class).put("recent", aRecentFiles.serialize());
				System.exit(0);
			}
		}
		else 
