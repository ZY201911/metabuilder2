...
public class WelcomeTab extends Tab
{
	...
	{
		VBox filesNamedElement = (VBox) ((HBox)((BorderPane) getContent()).getCenter()).getChildren().get(1);
		filesNamedElement.getChildren().remove(1, filesNamedElement.getChildren().size());
		for(NamedHandler handler : pFileOpenHanders)
		{
			Button fileShortcut = new Button(handler.getName());
			fileShortcut.setOnAction(handler);
			filesNamedElement.getChildren().add(fileShortcut);
		}
	}
